...
# -*- coding: utf-8 -*-
"""
Script de prueba usando Telethon (Cliente de Usuario)
Soporta archivos hasta 1.5GB según Stack Overflow
"""
...
from src.services.telegram_telethon_service import TelegramTelethonService

def test_video_upload(video_path: str, video_name: str, video_title: str, video_year: str = None) -> bool:
    """Prueba la subida de un video usando Telethon"""
    print(f"\n{'='*60}")
    ...
    # Crear servicio
    telegram_service = TelegramTelethonService()

    if not telegram_service.is_configured():
        print("❌ Telethon no está configurado")
        print("📋 Necesitas configurar:")
        print("   - TELEGRAM_API_ID")
        print("   - TELEGRAM_API_HASH") 
        print("   - TELEGRAM_PHONE")
        return False
    print("✅ Telethon configurado correctamente")

    print("🔍 Probando conexión con Telegram...")
    if not telegram_service.test_connection_sync():
    ...
    print("📤 Intentando subir video usando Telethon...")
...
def main():
    ...
    print("🚀 Script de Test con Telethon (Cliente de Usuario)")
    print("Basado en Stack Overflow: https://stackoverflow.com/questions/76383605/how-do-i-upload-large-files-to-telegram-in-python")
    print("=" * 80)
    ...
    print(f"   API ID: {'✅ Configurado' if settings.get('telegram.api_id') else '❌ No configurado'}")
    print(f"   API Hash: {'✅ Configurado' if settings.get('telegram.api_hash') else '❌ No configurado'}")
    print(f"   Phone: {'✅ Configurado' if settings.get('telegram.phone') else '❌ No configurado'}")
    print(f"   Channel ID: {'✅ Configurado' if settings.get_telegram_channel_id() else '❌ No configurado'}")
    
    if not all([
        settings.get('telegram.api_id'),
        settings.get('telegram.api_hash'),
        settings.get('telegram.phone'),
        settings.get_telegram_channel_id()
    ]):
        print("\n❌ Configuración incompleta. Necesitas añadir al .env:")
        print("   TELEGRAM_API_ID=tu_api_id")
        print("   TELEGRAM_API_HASH=tu_api_hash")
        print("   TELEGRAM_PHONE=tu_telefono")
        return
